...
export default class PopupPrice extends Component {
	...
	donePopup() {
		const { indexPopupTip } = this.props;
		const price = (this.refinput && this.refinput.value) ? this.refinput.value : "0.00"
		this.props.donePopupPrice(price, indexPopupTip);
	}

	componentDidMount() {
		const { valueTip } = this.props;
		this.setState({
			price: valueTip, isFocus: false
		});
	}

	componentWillReceiveProps(nextProps) {
		const { valueTip } = nextProps;
		this.setState({
			price: valueTip, isFocus: false
		});
	}
	...
	render() {
		const { isPopupTip } = this.props;
		const { price } = this.state;
		const width = this.getWidthInput();
		return (
			<PricePopup closeOnDocumentClick={false} open={isPopupTip} position="right center">
				<Container>
					<Header>Edit Tip</Header>
